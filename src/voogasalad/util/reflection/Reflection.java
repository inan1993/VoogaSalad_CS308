...
public class Reflection {
    ...
    public static Object callMethod (Class target, String name, Object... args) 
    		throws ReflectionException {
    	try {
    		for (Method current : target.getDeclaredMethods()) {
    			if (name.equals(current.getName())) {
    				Class<?>[] formals = current.getParameterTypes();
    				if (typesMatch(current,formals,args)) {
    					return current.invoke(null, convertArgs(current,formals,args));
    				}
    			}
    		}
    		throw new ReflectionException("No matching public method %s for %s", name, target.getName());
    	} catch (Exception e) {
    		throw new ReflectionException("No matching public method %s for %s", name, target.getName());
    	}
    }
